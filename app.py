...
if user_query := st.chat_input("Ask about today's news..."):
    ...
    if not user_api_key:
        ...
        with st.chat_message('user'):
         ...
         # toggle condition
        if use_search == True:
          system_prompt_text = """You are a live research assistant. You MUST use the web search tool to find the current information
                            before answering. Answer the query clearly
                             PRINT BASED ON INTERNET (IF YOU USE TOOL)"""
        else:
          system_prompt_text = """
                            You are a live research assistant.
                            any thing they will ask you will not use tool to answer
                            You cannot use the web search tool to find the current information
                           Answer the query clearly with your own memory.
                           PRINT BASED ON MY KNOWLEDGE (IF YOU DONT USE TOOL)
                           MAKE SURE NOT TO USE ANY TOOL"""
